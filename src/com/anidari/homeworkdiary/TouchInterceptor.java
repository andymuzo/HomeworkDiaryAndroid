...
public class TouchInterceptor extends ListView {
	...
	// used to determine which objects are moveable
	static final int todoListStartPosition = 8;
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (mDragListener != null || mDropListener != null) {
			...
			case MotionEvent.ACTION_DOWN:
				int x = (int) ev.getX();
				int y = (int) ev.getY();
				int itemnum = pointToPosition(x, y); // this gives the number in
														// the list of the
														// object

				HomeworkDetailListEntry hwk = (HomeworkDetailListEntry) this.getItemAtPosition(itemnum);
				
				if (itemnum == AdapterView.INVALID_POSITION
						|| hwk.getType() != TYPE.TO_DO_ENTRY) {
					break;
				}
				LinearLayout item = (LinearLayout) getChildAt(itemnum
	...
	private void doExpansion() {
		// inserted to disable movement for less sea sickness when dragging
		// too cowardly to completely get rid of it in case I change my mind!
		if (false) {
			...
			for (int i = 0;; i++) {
				...
				if (mDragPos < numheaders && i == numheaders) {
					...
					if (mDragPos == mSrcDragPos
						...
						// Ideally the item would be completely gone, but
						// neither
